      ...
      <div className={`py-3 px-3 border-b border-[var(--atlas-border)] flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} bg-[var(--atlas-surface)]/80 backdrop-blur-xs`}>
        {!isCollapsed ? (
          <div className="flex flex-col min-w-0 pr-1">
            <h1 className="text-xl font-black text-[var(--atlas-navy)] leading-tight tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>
              Atlas Auditor
            </h1>
            <span className="text-[9px] font-bold uppercase tracking-widest text-[var(--atlas-accent)] -mt-0.5">
              Inteligência Fiscal
            </span>
          </div>
        ) : (
          <img src="/favicon.svg" alt="Atlas Auditor Fiscal" className="w-10 h-10 rounded-lg shadow-xs object-contain shrink-0" title="Atlas Auditor Fiscal" />
        )}
        ...
        <div className="px-3 pt-3">
          <div className={`p-2.5 rounded-xl border flex items-center justify-between text-xs ${
            hasSped 
              ...
              : 'bg-[var(--atlas-surface)] border-[var(--atlas-border)] text-[var(--atlas-text-secondary)] shadow-xs'
          }`}>
            <div className="flex items-center space-x-2 truncate">
              ...
              <div className="truncate">
                ...
                <p className="text-xs text-[var(--atlas-text-muted)] truncate">
        ...
        <div className="px-3 pt-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[var(--atlas-surface)] border border-[var(--atlas-border)] rounded-lg pl-8 pr-7 py-1.5 text-xs text-[var(--atlas-text)] placeholder-[var(--atlas-text-muted)] focus:outline-none focus:border-[var(--atlas-navy)] transition-all shadow-xs"
            />
            {searchQuery && (
              <button 
      ...
      <div className="flex-1 px-2 py-3 space-y-3 overflow-y-auto custom-scrollbar">
        ...
        {searchQuery.trim() ? (
          <div className="space-y-1">
            <p className="px-2 text-xs font-semibold text-[var(--atlas-text-muted)] uppercase tracking-wider">
            ...
            <div className="space-y-1">
              {!isCollapsed ? (
                <button
                  ...
                  className="w-full flex items-center justify-between px-2 py-1 text-xs font-bold uppercase tracking-wider text-[var(--atlas-text-muted)] hover:text-[var(--atlas-text)] transition-colors"
                >
        ...
        {!isCollapsed ? (
          <>
            ...
            <div className="p-2 rounded-xl border border-[var(--atlas-border)] bg-[var(--atlas-surface)] space-y-1.5">
              <div className="flex items-center justify-between text-xs font-semibold text-[var(--atlas-navy)]">
                <span className="flex items-center gap-1">
                  <Cloud className="w-3.5 h-3.5 text-[var(--atlas-navy)] animate-pulse" />
                  <span>Backup Firestore</span>
                </span>
                {syncMsg && <span className="text-xs text-[var(--atlas-accent)] font-bold">{syncMsg}</span>}
              </div>
              <div className="grid grid-cols-2 gap-1">
                <button
                  onClick={handleManualBackup}
                  disabled={syncing}
                  className="atlas-btn atlas-btn-primary px-2 py-1 text-xs cursor-pointer"
                ...
                <button
                  ...
                  disabled={syncing}
                  className="atlas-btn atlas-btn-secondary px-2 py-1 text-xs cursor-pointer"
            ...
            <button
              ...
              className={`w-full flex items-center justify-center space-x-1.5 px-2 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                activeTab === 'settings' 
                  ? 'bg-[var(--atlas-navy)] text-white font-bold shadow-xs' 
