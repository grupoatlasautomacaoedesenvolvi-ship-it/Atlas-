...
export async function fetchGlobalStateTaxMatrix(escritorioId?: string): Promise<StateTaxRule[]> {
  ...
  // 1. Try Backend API (Admin SDK) — o servidor resolve o escritório a partir
  // do usuário autenticado; nunca mandamos mais um ID sentinela.
  try {
    ...
    if (res.ok) {
      ...
      if (data.success && Array.isArray(data.rules) && data.rules.length > 0) {
        if (escritorioId) localStorage.setItem(`atlas_state_tax_matrix_${escritorioId}`, JSON.stringify(data.rules));
  ...
  // 2. Try Client Firestore — só com um escritorioId real. Nunca usar um ID
  // sentinela como 'escritorio-default': as regras do Firestore já negam
  // esse acesso por design.
  if (escritorioId) {
    try {
      const docRef = doc(db, 'escritorios', escritorioId, 'config', 'matriz_tributaria');
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
        const dataRules = docSnap.data().rules;
        if (Array.isArray(dataRules) && dataRules.length > 0) {
          localStorage.setItem(`atlas_state_tax_matrix_${escritorioId}`, JSON.stringify(dataRules));
  ...
  // 3. Try LocalStorage
  try {
    const local = (escritorioId && localStorage.getItem(`atlas_state_tax_matrix_${escritorioId}`)) || localStorage.getItem('atlas_state_tax_matrix');
    if (local) {
  ...
  // 4. Save and return defaults — só persiste se houver um escritório real.
  if (escritorioId) {
    await saveGlobalStateTaxMatrix(DEFAULT_TAX_RULES, escritorioId);
  }
...
export async function saveGlobalStateTaxMatrix(rules: StateTaxRule[], escritorioId?: string): Promise<void> {
  ...
  try {
    ...
    if (escritorioId) localStorage.setItem(`atlas_state_tax_matrix_${escritorioId}`, JSON.stringify(rules));
  } catch (err) {
  ...
  // só quando houver um escritório real.
  if (!savedToDb && escritorioId) {
    try {
